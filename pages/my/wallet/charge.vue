...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            charge(){
			//	plus.runtime.openURL('https://www.php.cn/uni-app/466227.html');  
                //1支付宝 2微信
				let payUrl = "";
				console.log(this.amount)
                _get.charge({amount:this.amount,charge_type:4},function (ret) {
				    payUrl = ret.orderInfo.payUrl;
					console.log("订单信息",ret)
					if(payUrl != ""){
						uni.showToast({
							icon:"none",
							title:"即将前往浏览器支付！"
						});
						console.log("跳转网址",payUrl)
						setTimeout(() => {
							// uni.navigateTo({
							// 	url:'show_otherpay?url='+payUrl
							// })
