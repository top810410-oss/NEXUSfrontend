<template>
	<view class="zai-box" style="background-image: url(../../static/qdy.png);background-size: 100% 100%;
	   ...
	   height: 800px;">
		...
		<view class="zai-form">
			...
			<input class="zai-input" v-show="needInvitionCode" v-model="form.invitationCode" type="password" placeholder="请输入邀请码"/>	
	...
	export default {
		...
		onShow(){
			...
			_get.getDbSysConfig({
				...
				if(res.value == 1){
					_this.needInvitionCode = 1
				}else {
					_this.needInvitionCode = 0
				}
				console.log("全局邀请码状态",_this.needInvitionCode);
			})
		...
		computed: {
			checkIn () {
				if(this.needInvitionCode){
					return this.form.password != '' && this.form.username != ''
					&& this.form.password.length > 5 && this.form.username.length > 5
					&& this.form.invitationCode != '' && this.form.invitationCode.length > 3;
				}
				return this.form.password != '' && this.form.username != '' 
				&& this.form.password.length > 5 && this.form.username.length > 5;
			}
