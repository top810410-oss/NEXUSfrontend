    ...
    <view class="moments__post"
      ...
      <view class="post_right">
        ...
        <view class="toolbar">
		  ...
		  <view style="font-size: 20upx;margin-left: 150upx;" @tap="del(index)" v-if="my_data.id==post.uid">删除</view>
          <view class="like"
                @tap="like(index)">
	  ...
	  _this.$httpSend({
	    ...
	    success (data) {
		  if(data.err==0){
			  ...
			  let circle_data = _data.localData('circle_data');
			  uni.removeStorage({key: 'circle_data'});
			  circle_data.splice(index,1);
			  console.log('json--'+JSON.stringify(circle_data));
			  uni.setStorage({
			  	key: 'circle_data',
			  	data: circle_data
			  });
