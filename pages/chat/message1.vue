...
<page-meta>
  ...
  <view class="page"
		...
		<!-- <uni-nav-bar style="line-height: 2px;" left-icon="left" right-text="菜单" left-text="返回"  :status-bar="true" @clickLeft="back">
			<view style="width: 100%;text-align: center;line-height: 15px;">
				<text style="font-size: 18px;">123</text>
				<text>456</text>
			</view>
		</uni-nav-bar> -->
                  ...
                  <image src="/static/img/red-envelope-chat.png"
						 ...
						 <block v-if="row.msg.content.rid_user_id == my_data.id && row.msg.content.user_id == my_data.id">
						        ...
						        <block v-else-if="row.msg.content.rid_user_id != my_data.id && row.msg.content.user_id == my_data.id">
						         我领取了{{row.msg.content.ridnickname}}发出的
						        </block>
						        <block v-else-if="row.msg.content.rid_user_id == my_data.id && row.msg.content.user_id != my_data.id">
            ...
            <block v-if="row.type==0">
              ...
              <view class="my"
                ...
                <view class="left"
                  ...
                  <view v-if="row.msg.type==0"
                        class="bubble">
                  						<rich-text v-if="row.msg.hasEmoji == 1" :nodes="row.msg.content.text" space="true" />
                  						<text v-else>{{row.msg.content.text}}</text>
                  </view>
                  <!-- 语言消息 -->
...
const TUICalling = uni.requireNativePlugin("TUICallingUniPlugin-TUICallingModule");
  ...
  export default {
    ...
    data () {
      return {
		  ...
		  nbTitle: '默认用户名',
		          ...
		          titleIconRadius: '10px',
		          subtitleText: '0.0.0.0',
		          nbLoading: false,
		          nbFrontColor: '#000000',
    ...
    onShow() {
	  ...
	  console.log('聊天信息对方userid：',chat_data.obj_id)
      console.log('聊天信息对方名字：',chat_data.show_name)
	  _this.objId = chat_data.obj_id;
	  console.log("音视频对方id",_this.objId);
	  
      if (chat_data && chat_data.list.length) {
        _this.scrollAnimation = false;
        console.log('chat_data.show_name',chat_data.show_name)
		console.log("客户端Ip", _this.clientIp);
		_get.getClientIp({}, function(res) {
			console.log("客户端Ip信息", res);
			_this.clientIp = res.ip;
			_this.subtitleText = _this.clientIp;
			_this.nbTitle = chat_data.show_name;
			// #ifdef H5
			// uni.setNavigationBarTitle({
			//   title: chat_data.show_name+'('+_this.clientIp+')',
			// });
			// #endif
      ...
      uni.$on('data_chat_data_push', function (data) {
        ...
        _this.noMore = 0;
        data = _this.msgDataHandle(data);
	//	console.log("测试数据2",JSON.stringify(data));
    ...
    methods: {
      ...
      getVideo (data) {
        let _this = this;
        // #ifdef APP-PLUS
		//console.log('测试通话数据',data);
		console.log('通话对方id',_this.objId);
		console.log('通话类型',data.callType);
		_this.myId = data.userid;
		_this.myUserSig = data.usersig;
		
		this.doubleClick = false;
		TUICalling.call({userID: _this.objId, type: data.callType == 'video'? 2 : 1})
		
		// TUICalling.login({
		//  					    sdkAppID: 1400594375, 
		//  					    userID: _this.myId,
		//  					    userSig: _this.myUserSig,
		...
		// 	avatar: data.myavatar,
		// 	appid: "1400593168",
		// 	appsign: data.usersig,
		// 	calltype: data.callType = 'video'?'1':'0',
		// 	beauty: "1",
		// 	whiteness: "1",
		// 	ruddiness: "1"
		// }, result => {
      ...
      msgDataHandle (data, type) {
		  console.log("测试数据5",JSON.stringify(data));
        for (let i = 0, j = data.length, msg_img; i < j; i++) {
          if (data[i].type == 0 && data[i].msg.type == 2) {
			...
			if(this.hasEmoji == 1){
				this.$set(data[i].msg,'hasEmoji',1);
				this.hasEmoji = 0;
			}
          }
        }
		console.log("测试数据6",JSON.stringify(data));
        return data;
      },
      addMsgImgList (data) {
        for (let i = 0, j = data.length, msg_img; i < j; i++) {
      ...
      msgAction (id, user_id, msg_text) {
        let _this = this;
		console.log("测试数据",msg_text);
		//console.log("测试数据2",_this.getHtml(msg_text));
		
		//var text = (_this.getHtml(msg_text))[0].children[0].text;
		var text = msg_text;
		//console.log(text);
      ...
      sendMsg (content, type) {
		...
		if(reg.test(this.textMsg.replace(/</g, '&lt;'))){
		    console.log("含有一个或多个汉字");
		    
		}else{
			console.log("不是含有一个或多个汉字");
