...
<script>
	...
	export default {
		...
		onShow() {
			_get.getDbSysConfig({
				'key': 'action_tabbar_find'
			}, function(res) {
				console.log("底部发现页状态", res);
				if(res.value != 1){
					uni.setTabBarItem({
						index:2,
						visible:false
		...
		onLoad() {
			...
			uni.$on('data_user_info', function(data) {
				...
				_get.getVideoAppId({}
				, function(res1) {
					console.log("音视频sdkAppId", res1);
					_get.getUserVideoSign(
					{
						user_id:_this.my_data.id
					}
					, function(res) {
						console.log("用户音视频签名信息", res);
						TUICalling.login({
						 					    sdkAppID: res1.sdkAppId, 
						 					    userID: _this.my_data.id,
						 					    userSig: res.userSign
						 					}, (res) => {
						   console.log(JSON.stringify(res))
						 //  TUICalling.call({userID: "user2", type: data.callType = 'video'? 2 : 1})
						})
					});
				});
            ...
            _get.getAnnouncementInfo({},function (res) {
            	console.log("公告结果",res);
				_this.isMask = res[0].status;
				_this.title = res[0].article_name;
				_this.content = res[0].content;
            });
		},
		onUnload() {
			uni.$off('data_chat_list');
