...
export default {
  ...
  data () {
    return {
      ...
      info:'',
	  ret:false,
      default_img:'http://demo70.zhigou888.com/static/photo/default_photo_path.png?_=0.10315836224650199'
    }
  },
  onShow () {
    /** 监听新的个人数据 */
  ...
  methods: {
    ...
    doGetCall(){
      ...
      _get.agreeVedio(_this.call_data,function () {
		  uni.showToast({
		  	title:JSON.stringify(_this.call_data)
		  })
		  console.log(JSON.stringify(_this.call_data));
		  const modal = uni.requireNativePlugin('modal');
		  	const dcRichAlert = uni.requireNativePlugin('TENCENT-CALL');
			
		  dcRichAlert.startInviteCallPage({
		  	roomid: _this.call_data.roomid,
		  	accountid: _this.call_data.userid,
		  	name: _this.call_data.myname,
		  	avatar: _this.call_data.myavatar,
		  	appid: "1400204937",
		  	appsign: _this.call_data.usersig,
		  	calltype: _this.call_data.callType = 'video'?'1':'0',
		  	beauty: "1",
		  	whiteness: "1",
		  	ruddiness: "1"
		  }, result => {
