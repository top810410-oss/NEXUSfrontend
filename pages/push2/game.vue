<template>
  ...
  <view class="page">
	  ...
	  <view :class="isActive = 1 ? 'activeClass' : 'displayClass'">
...
export default {
  ...
  onShow () {
	  ...
	  uni.$on('data_user_info', function (data) {
	    _this.my_user_id = data.id;
	     console.log("用户个人数据：",_this.my_user_id);
	  });
	  _this.isActive = 1;
  ...
  methods: {
	  ...
	  goCircle(path){
	      ...
	      if(path){
	          uni.navigateTo({
	              url: path+'?user_id='+_this.my_user_id
	          });
	      }
	  },
      goScanCode () {
